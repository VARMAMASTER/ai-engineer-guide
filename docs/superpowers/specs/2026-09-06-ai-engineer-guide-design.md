...
Day        { id, number: 1..30, weekId, kind: 'weekday'|'weekend'|'special',
             tasks: DayTask[] }
DayTask    { refId: string, track: 'dsa'|'study-sd'|'study-ml'|'reading'|'build'|'review',
...
`DayTask.refId` must resolve to a `DsaProblem`, `SdQuestion`, `SdPattern`, `TopicQuestion`, `Topic`, `Milestone`, or `Reading`, or be one of the fixed review ids `review-week`, `review-mock`, `review-retro`, `review-publish`.
...
Weekly totals: DSA 5h (10 problems), SD 3h, AI/ML 3h, reading 1.5h, build 10h. Sum 22.5h. Validation allows 21.5 to 23.5.
...
- 7 defense documents written: problem, architecture, ml, tradeoffs, scaling, failures, interview_questions.
...
- Any week's tasks (days 1–28) sum outside 21.5 to 23.5 hours, summing `DayTask.minutes` only.
